...
def instantiate_guesser(guesser_type, flags, load):
    ...
    if guesser_type == "GprGuesser":
        ...
        if load:
            guesser.load()
        if flags.TfidfGuesser_filename:                                              #sol   
            guesser.init_retriever("tfidf", flags.TfidfGuesser_filename)             #sol   
        if flags.WikiGuesser_filename:                                               #sol   
            guesser.init_retriever("wiki", flags.WikiGuesser_filename)               #sol           
    ...
    if guesser_type == "DanGuesser":                                
        ...
        if load:                                                    
            guesser.load()                                          
    if guesser_type == "WikiGuesser":                               #sol
        from wiki_guesser import WikiGuesser                        #sol        
        guesser = WikiGuesser(flags.WikiGuesser_filename)           #sol
        if load:                                                    #sol
            guesser.load()                                          #sol
...
def load_buzzer(flags, load=False):
    ...
    if flags.buzzer_type == "BoostedTreeBuzzer":                                                            #sol   
        from boosted_tree_buzzer import BoostedTreeBuzzer                                                   #sol   
        buzzer = BoostedTreeBuzzer(flags.BoostedTreeBuzzer_filename, flags.run_length, flags.num_guesses)   #sol
    ...
    for ff in flags.features:
        ...
        if ff == "Frequency":                                      #sol
            from features import FrequencyFeature                  #sol
            feature = FrequencyFeature(ff)                         #sol
            feature.add_training("data/qanta.guesstrain.json.gz")  #sol
            buzzer.add_feature(feature)                            #sol
    return buzzer
