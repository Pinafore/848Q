...
def add_buzzer_params(parser):
    ...
    from rl_lorabert_buzzer import LoraBertRLParameters
    
    params = {}
    params["logistic"] = LogisticParameters()
    params["lorabert"] = LoraBertParameters()
    params["rl_lorabert"] = LoraBertRLParameters()
...
def load_buzzer(flags, guesser_params, load=False):
    ...
    if flags.buzzer_type == 'rl_lorabert':
        from rl_lorabert_buzzer import LoRABertRLBuzzer
        buzzer = LoRABertRLBuzzer(filename=flags.lorabert_buzzer_filename, run_length=flags.run_length, num_guesses=1, cfg=flags)
        buzzer.initialize_model(flags.rl_lorabert_buzzer_base_model, flags.rl_lorabert_buzzer_rank, flags.rl_lorabert_buzzer_alpha)
