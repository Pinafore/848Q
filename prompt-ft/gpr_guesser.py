...
def clean_probs(log_probs):
    if isinstance(log_probs, float):
        return log_probs
    elif isinstance(log_probs, int):
        confidence = float(log_probs)
...
class GprGuesser(Guesser):
    ...
    def __init__(self, cache_filename, num_examples=2, num_shards=9999, shard_vocab=ascii_lowercase, shard_prefix_length=5, save_every=2):
        """
