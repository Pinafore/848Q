...
2. improve the expected wins of the finetuned over either logistic regression with just the confidence feature and length or logistic regression with just the confidence feature and the guess (in other words, a Threshold Buzzer that you've tuned a little).  (This will depend on your Guesser, obviously)
...
*A:* Start on DSPy as a guesser and then use a Threshold buzzer on top of that.  This will be a good baseline and relatively simple.  But if DSPy collapses your confidences and gets rid of the distribution, a threshold buzzer will be less useful.
