...
====================================
We have a script `eval.py` to judge both parts of the code: the guesser and the buzzer.
...
Now you need to evaluate the buzzer.  The script eval.py will run the classifier on all of your data and then record the outcome.  There are several things that could happen:
...
First, you'll need to run the eval script.  You'll set the ThresholdBuzzer parameters from the command line:
